...
const trainer = {
  ...
  talk: function () {
    logToConsoleBox("Pikachu! I choose you!");
  },
};
...
Pokemon.prototype.tackle = function (targetPokemon) {
  ...
  logToConsoleBox(
    `${this.name} used Tackle on ${targetPokemon.name}. ${targetPokemon.name}'s health is now ${targetPokemon.health}.`
  );
...
Pokemon.prototype.faint = function () {
  logToConsoleBox(`${this.name} has fainted. You won!`);
...
Pokemon.prototype.increaseLevel = function (levelIncrease) {
  ...
  logToConsoleBox(`${this.name}'s level increased to ${this.level}.`);
  logToConsoleBox(`${this.name}'s attack increased to ${this.attack}.`);
  logToConsoleBox(`${this.name}'s health increased to ${this.health}.`);
};
...
const tackleHandler = function () {
  ...
  if (enemyPokemon.health <= 0) {
    enemyPokemon.faint();
    logToConsoleBox(`You defeated ${enemyPokemon.name}!`);
    document.getElementById("new-battle-btn").style.display = "inline-block";
  } else {
    logToConsoleBox(
      `${enemyPokemon.name}'s health is now ${enemyPokemon.health}.`
    );
  }
};
...
const battleHandler = function () {
  if (battleOccurring === 0) {
    ...
    logToConsoleBox(`${myPokemon.name} is battling ${enemyPokemon.name}.`);
    logToConsoleBox(`${myPokemon.name} used Tackle on ${enemyPokemon.name}.`);
    displayPokemonInfo(enemyPokemon);
  }
};
...
document
  .getElementById("new-battle-btn")
  .addEventListener("click", newBattleHandler);
